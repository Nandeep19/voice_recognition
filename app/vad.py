...
Model is loaded once via ``torch.hub`` (~2 MB download on first run) and reused
across all requests through a thread-safe singleton.
"""
...
def _get_vad():
    """Lazy-load Silero VAD model (thread-safe singleton)."""
...
def detect_speech_segments(
    ...
    *,
    ...
    # Silero requires 16 kHz
    assert sample_rate == 16_000, f"Silero VAD requires 16 kHz, got {sample_rate}"
